...
namespace VVVV.Nodes
{
	...
	public class EX9_GeometryVVVVultureNode : IPluginEvaluate
	{
		...
		[Output("Normals3")]
		public ISpread<Vector3> Fnormals3;
			...
			if (Fenabled[0]){
				try{
					...
					if (Vulture != null){
						...
						Fvertices3.SliceCount = Vulture.meshes[x].verticesVec3.Length;
						Fnormals3.SliceCount = Vulture.meshes[x].normalsVec3.Length;
						Findices.SliceCount = Vulture.meshes[x].indices.Length;
						
						for (int i =0; i < Vulture.meshes[x].verticesVec3.Length; i++)
						{
							Fvertices3[i] = Vulture.meshes[x].verticesVec3[i];
							Fnormals3[i] = Vulture.meshes[x].normalsVec3[i];
							Findices[i] = Vulture.meshes[x].indices[i];
						}
						//FOutput[0] = Vulture.meshes[x].indices.Length.ToString();
						FOutput[0] = Findices.SliceCount.ToString();
					}
				}
				catch{}
			}
